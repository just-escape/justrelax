<template>
  <div class="window d-flex flex-column" :style="{left: left, bottom: bottom, transform: transform}">
    <div class="d-flex flex-row rounded-top bg-info shadow-box-top align-items-center h-header">
      ...
      <h3 class="size-12 py-1 mx-3 mb-0 text-light">{{ $t('ORDER') }}</h3>
      <Neons class="mr-2" color="var(--light)"/>
    </div>
    <div class="border-left-bottom-right shadow-box-top bg-light-transparent p-3">
      <div class="media">
        <div class="mr-3 align-self-center d-flex justify-content-center align-items-center" style="width: 50px; height: 50px; border-radius: 50%">
...
export default {
  name: "ProcessingOrderNotification",
  components: {
    Neons,
  },
  data() {
...
.loading {
  ...
  width: 40px;
  height: 40px;
  border: 8px solid rgba(23, 162, 184, .5);
  border-radius: 50%;
